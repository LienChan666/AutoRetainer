...
public sealed unsafe class ExchangeLists : InventoryManagementBase
{
    ...
    public override void Draw()
    {
        ...
        ImGuiEx.TextWrapped("Purchase Logic:".Loc());
        ImGuiEx.TextWrapped("- The system will attempt to purchase the first available item from the list.".Loc());
        ImGuiEx.TextWrapped("- Purchases will continue until the quantity of that item in your inventory reaches the specified target amount.".Loc());
        ImGuiEx.TextWrapped("If no listed items are available for purchase, or they cannot fit into your inventory:".Loc());
        ImGuiEx.TextWrapped("- The system will purchase Ventures instead.".Loc());
        ImGuiEx.TextWrapped("- Venture purchases will continue until your Venture count reaches 65,000.".Loc());
        ImGuiEx.TextWrapped("Once the Venture cap is reached and no other purchases are possible:".Loc());
        ImGuiEx.TextWrapped("- Any excess Grand Company Seals will be discarded.".Loc());
    ...
    {
        static string GetRankName(uint rank)
        {
            return rank < Utils.GCRanks.Length ? Utils.GCRanks[rank] : rank.ToString();
        }
        ...
        }, () =>
        {
            ...
            ImGui.InputInt("Seals to keep".Loc(), ref plan.RemainingSeals.ValidateRange(0, 70000), 0, 0);
        ...
        if(ImGui.BeginCombo("##Add Items", "Add Items".Loc(), ImGuiComboFlags.HeightLarge))
        {
            ...
            foreach(var x in Utils.SharedGCExchangeListings)
            {
                if(getFilter2().Length > 0
                    ...
                    && !GetRankName(x.Value.MinPurchaseRank).Equals(getFilter2(), StringComparison.OrdinalIgnoreCase)
                    ) continue;
        ...
        if(ImGuiEx.BeginDefaultTable("GCDeliveryList", ["##dragDrop", $"~{"Item".Loc()}", "GC".Loc(), "Lv".Loc(), "Price".Loc(), "Category".Loc(), "Keep".Loc(), "One-Time".Loc(), "##controls"]))
        {
            for(var i = 0; i < plan.Items.Count; i++)
            {
                ...
                if(!Utils.SharedGCExchangeListings.TryGetValue(currentItem.ItemID, out var meta))
                {
                    ...
                    ImGuiEx.TextV("?");
                    ImGui.TableNextColumn();
                    ImGuiEx.TextV(string.Format("Unknown item ID: {0}".Loc(), currentItem.ItemID));
                    for(var c = 0; c < 6; c++)
                    {
                        ImGui.TableNextColumn();
                        ImGuiEx.TextV("-");
                    ...
                    ImGuiEx.Tooltip("Remove this unknown item from plan".Loc());
                    ImGui.PopID();
                    continue;
                }
                ...
                if(getFilter().Length > 0
                    ...
                    && !GetRankName(meta.MinPurchaseRank).Equals(getFilter(), StringComparison.OrdinalIgnoreCase)
                    ) continue;
                ...
                if(Svc.Data.GetExcelSheet<GrandCompanyRank>().TryGetRow(meta.MinPurchaseRank, out var rank) && ThreadLoadImageHandler.TryGetIconTextureWrap(rank.IconFlames, false, out var tex))
                {
                    ...
                    var rankName = GetRankName(meta.MinPurchaseRank);
                ...
                ImGuiEx.Tooltip("Select amount of items to purchase once. Whenever purchase is made on any character using this plan, an amount will be subtracted from this value. Once it reaches 0, it will return to the \"Keep\" amount.".Loc());
                ImGui.TableNextColumn();
