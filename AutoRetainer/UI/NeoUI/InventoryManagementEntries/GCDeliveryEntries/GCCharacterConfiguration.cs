...
{
    public override string Name { get; } = "筹备稀有品/角色配置";

    public override int DisplayPriority => -10;

    public override void Draw()
    {
        ImGuiEx.TextWrapped($"你可以在此为已登记角色分配预设兑换列表，并选择交纳模式。");
        ImGuiEx.SetNextItemFullWidth();
        ImGuiEx.FilteringInputTextWithHint("##search", "搜索...", out var filter);
        if(ImGuiEx.BeginDefaultTable(["~角色", "方案", "交纳模式"]))
        {
            foreach(var characterData in C.OfflineData)
            {
                ...
                if(ImGui.BeginCombo("##chPlan", plan?.DisplayName ?? "默认方案", ImGuiComboFlags.HeightLarge))
                {
                    if(ImGui.Selectable("默认方案", plan == null)) characterData.ExchangePlan = Guid.Empty;
                    ImGui.Separator();
