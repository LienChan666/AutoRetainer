...
public unsafe class InventoryManagementCommon
{
    ...
    public void DrawListNew(Action<uint> addAction, Action<uint> removeAction, IReadOnlyList<uint> itemList, Action<uint> additionalButtons = null, Predicate<Item> filter = null)
    {
        ...
        ImGuiEx.TreeNodeCollapsingHeader("Mass addition/removal".Loc(), () =>
        {
            ...
            if(ImGui.BeginCombo("Select Categories".Loc(), SelectedCategories.Count != 0 ? $"{SelectedCategories.Count} {"selected".Loc()}" : "None selected".Loc(), ImGuiComboFlags.HeightLarge))
            {
            ...
            {
                ImGui.SetNextItemWidth(200f);
                Modified |= ImGui.InputText("Filter by name".Loc(), ref ItemSearch, 100);
                ImGui.SetNextItemWidth(200f);
                if(ImGui.BeginCombo("Select rarity".Loc(), Rarities.Any() ? $"{Rarities.Print()}" : "Any rarity".Loc(), ImGuiComboFlags.HeightLarge))
                {
                ...
                ImGui.SetNextItemWidth(200f);
                Modified |= ImGui.InputInt("Minimum item level".Loc(), ref ItemLevelMin);
                ImGui.SetNextItemWidth(200f);
                Modified |= ImGui.InputInt("Maximum item level".Loc(), ref ItemLevelMax);
                ...
                if(ImGuiEx.CollapsingHeader(string.Format("Selected {0} items, among which {1} already present in list".Loc(), SelectedItems.Count, SelectedItems.Count(x => itemList.Contains(x.RowId))) + "###counter"))
                {
