...
public unsafe class InventoryManagementCommon
{
    ...
    public void DrawListNew(Action<uint> addAction, Action<uint> removeAction, IReadOnlyList<uint> itemList, Action<uint> additionalButtons = null, Predicate<Item> filter = null)
    {
        ...
        ImGuiEx.TreeNodeCollapsingHeader("批量添加/移除", () =>
        {
            ...
            if(ImGui.BeginCombo("选择分类", SelectedCategories.Count != 0 ? $"已选 {SelectedCategories.Count} 项" : "未选择", ImGuiComboFlags.HeightLarge))
            {
            ...
            if(SelectedCategories.Count > 0)
            {
                ...
                ImGui.SetNextItemWidth(200f);
                if(ImGui.BeginCombo("选择稀有度", Rarities.Any() ? Rarities.Select(x => Lang.ItemRarityNames[x]).Print() : "任意稀有度", ImGuiComboFlags.HeightLarge))
                {
                    foreach(var r in Enum.GetValues<ItemRarity>())
                    {
                        Modified |= ImGuiEx.CollectionCheckbox(Lang.ItemRarityNames[r], r, Rarities);
                    }
                    ImGui.EndCombo();
                }
                ImGui.SetNextItemWidth(200f);
                Modified |= ImGui.InputInt("最低物品品级", ref ItemLevelMin);
                ImGui.SetNextItemWidth(200f);
                Modified |= ImGui.InputInt("最高物品品级", ref ItemLevelMax);
                ...
                if(ImGuiEx.CollapsingHeader($"已选 {SelectedItems.Count} 件物品，其中 {SelectedItems.Count(x => itemList.Contains(x.RowId))} 件已在列表中###counter"))
                {
                ...
                ImGuiEx.Tooltip("按住 CTRL 并点击");
                if(ImGuiEx.IconButtonWithText(FontAwesomeIcon.MinusSquare, "将这些物品从列表中移除", ImGuiEx.Ctrl))
                {
                    foreach(var x in SelectedItems)
                    {
                        removeAction(x.RowId);
    ...
    public void ImportFromArDiscard(List<uint> target)
    {
        ...
        ImGuiEx.HelpMarker("如果你使用 Discard Helper 插件，可通过此按钮导入条目并与现有条目合并。按住 CTRL 并点击执行。");
    }
