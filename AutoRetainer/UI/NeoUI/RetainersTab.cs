...
public class RetainersTab : NeoUIEntry
{
    ...
    private void MassConfigurationChangeWidget()
    {
        ...
        if(ImGui.BeginCombo("##sel", $"已选 {SelectedRetainers.Count}", ImGuiComboFlags.HeightLarge))
        {
        ...
        if(ImGuiEx.IconButtonWithText((FontAwesomeIcon)61543, "按等级将雇员加入选择"))
        {
            foreach(var x in C.OfflineData)
            {
                foreach(var r in x.RetainerData)
        ...
        ImGui.SameLine();
        if(ImGuiEx.IconButtonWithText((FontAwesomeIcon)62073, "启用规划器并指定探险方案"))
        {
            var num = 0;
        ...
        ImGui.SameLine();
        if(ImGuiEx.IconButtonWithText((FontAwesomeIcon)62566, "设置转存方案"))
        {
            var num = 0;
            foreach(var x in SelectedRetainers)
            {
        ...
        if(ImGuiEx.IconButtonWithText((FontAwesomeIcon)61526, "从已选雇员移除转存方案"))
        {
            var num = 0;
            foreach(var x in SelectedRetainers)
            {
        ...
        if(ImGuiEx.IconButtonWithText((FontAwesomeIcon)61526, "禁用已选雇员的探险规划器"))
        {
            var num = 0;
            foreach(var x in SelectedRetainers)
            {
        ...
        if(ImGuiEx.IconButtonWithText((FontAwesomeIcon)61452, "启用已选雇员"))
        {
            var num = 0;
            foreach(var x in SelectedRetainers)
            {
        ...
        if(ImGuiEx.IconButtonWithText((FontAwesomeIcon)61453, "禁用已选雇员"))
        {
            var num = 0;
            foreach(var x in SelectedRetainers)
            {
        ...
        if(ImGuiEx.IconButtonWithText((FontAwesomeIcon)61528, "为已选雇员所属角色启用雇员多角色模式"))
        {
            var num = 0;
            foreach(var x in SelectedRetainers)
            {
        ...
        if(ImGuiEx.IconButtonWithText((FontAwesomeIcon)61527, "为已选雇员所属角色禁用雇员多角色模式"))
        {
            var num = 0;
            foreach(var x in SelectedRetainers)
            {
