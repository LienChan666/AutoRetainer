...
internal class VentureBrowser : Window
{
    ...
    public override void Draw()
    {
        ...
        if(SelectedRetainer != null && SelectedCharacter != null)
        {
            ...
            {
                ImGuiEx.TextCentered($"{Lang.CharLevel}{SelectedRetainer.Level} {ExcelJobHelper.GetJobNameById(SelectedRetainer.Job)} | {"Gathering".Loc()}: {adata.Gathering} ({adata.Gathering / (float)MaxGathering:P0}) | {"Perception".Loc()}: {adata.Perception} ({adata.Perception / (float)MaxPerception:P0})");
            }
            else
            {
                ImGuiEx.TextCentered($"{Lang.CharLevel}{SelectedRetainer.Level} {ExcelJobHelper.GetJobNameById(SelectedRetainer.Job)} | {"Item Level".Loc()}: {adata.Ilvl} ({adata.Ilvl / (float)MaxPerception:P0})");
            }
