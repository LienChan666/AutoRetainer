...
internal class VentureBrowser : Window
{
    ...
    public VentureBrowser() : base("探险管理器")
    {
    ...
    public override void Draw()
    {
        ...
        if(ImGui.BeginCombo("##selectRet", SelectedCharacter != null ? $"{Censor.Character(SelectedCharacter.Name, SelectedCharacter.World)} - {Censor.Retainer(SelectedRetainer.Name)} - {SelectedRetainer.Level} {ExcelJobHelper.GetJobNameById(SelectedRetainer.Job)}" : "选择一个雇员...", ImGuiComboFlags.HeightLarge))
        {
            foreach(var x in C.OfflineData.OrderBy(x => !C.NoCurrentCharaOnTop && x.CID == Player.CID ? 0 : 1))
            {
                foreach(var r in x.RetainerData)
                {
                    if(ImGui.Selectable($"{Censor.Character(x.Name, x.World)} - {Censor.Retainer(r.Name)} - 等级 {r.Level} {ExcelJobHelper.GetJobNameById(r.Job)}"))
                    {
                        SelectedRetainer = r;
                        SelectedCharacter = x;
                        Reset();
        ...
        if(SelectedRetainer != null && SelectedCharacter != null)
        {
            ...
            {
                ImGuiEx.TextCentered($"{Lang.CharLevel}{SelectedRetainer.Level} {ExcelJobHelper.GetJobNameById(SelectedRetainer.Job)} | 平均品级：{adata.Ilvl}（{adata.Ilvl / (float)MaxPerception:P0}）");
            }
            ...
            {
                ImGuiEx.TextWrapped($"该雇员暂无数据。请打开传唤铃并选中该雇员以生成数据。");
            }
            else
            {
                if(Data.Count == 0)
                ...
                if(ImGui.BeginTable("##VentureBrowserTable", 9, ImGuiTableFlags.SizingFixedFit | ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg | ImGuiTableFlags.ScrollY))
                {
                    ...
                    foreach(var x in Data.Where(x => x.VentureName.Contains(search, StringComparison.OrdinalIgnoreCase) && x.VentureLevel >= minLevel && x.VentureLevel <= maxLevel))
                    {
                        ...
                        if(ImGui.SmallButton($"添加到规划器##{x.ID}"))
                        {
                            adata.VenturePlan.List.Add(new(x.ID));
                        }
                        ImGui.SameLine();
                        if(ImGui.SmallButton($"检查价格##{x.ID}"))
                        {
