...
public sealed class VenturePlanner : Window
{
    ...
    public override void Draw()
    {
        ...
        var selectedText = SelectedCharacter != null && SelectedRetainer != null
            ? $"{Censor.Character(SelectedCharacter.Name, SelectedCharacter.World)} - {Censor.Retainer(SelectedRetainer.Name)} - {Lang.CharLevel}{SelectedRetainer.Level} {ExcelJobHelper.GetJobNameById(SelectedRetainer.Job)}"
            : "Select a retainer...".Loc();
        if(ImGui.BeginCombo("##selectRet", selectedText, ImGuiComboFlags.HeightLarge))
        {
            foreach(var x in C.OfflineData.OrderBy(x => !C.NoCurrentCharaOnTop && x.CID == Player.CID ? 0 : 1))
            {
                foreach(var r in x.RetainerData)
                {
                    if(ImGui.Selectable($"{Censor.Character(x.Name, x.World)} - {Censor.Retainer(r.Name)} - {Lang.CharLevel}{r.Level} {ExcelJobHelper.GetJobNameById(r.Job)}"))
                    {
                        SelectedRetainer = r;
                        SelectedCharacter = x;
                    }
        ...
        if(SelectedRetainer != null && SelectedCharacter != null)
        {
            ...
            /*ImGuiEx.TextV("Share venture plan with:".Loc());
            ImGui.SameLine();
            ImGuiEx.SetNextItemFullWidth();
            ...
            if(adata.LinkedVenturePlan == "")
            {
                ...
                if(C.SavedPlans.Count > 0)
                {
                    ...
                    if(ImGui.BeginCombo("##load", "Load saved plan...".Loc(), ImGuiComboFlags.HeightLarge))
                    {
                        ...
                        for(var i = 0; i < C.SavedPlans.Count; i++)
                        {
                            ...
                            {
                                ImGui.OpenPopup("Context".Loc());
                            }
                            if(ImGui.BeginPopup("Context".Loc()))
                            {
                ...
                if(adata.VenturePlan.List.Count > 0)
                {
                    ...
                    }, delegate
                    {
                        if(ImGuiEx.IconButton(FontAwesomeIcon.Save))
                        {
                            ...
                            Notify.Success($"{adata.VenturePlan.Name}: {"Plan saved".Loc()}");
                        }
                        ImGuiEx.Tooltip((overwrite ? "Overwrite Existing Venture Plan" : "Save Venture Plan").Loc());
                    });
                }
                ...
                if(C.Verbose)
                {
                    ...
                    {
                        ImGuiEx.InputUint("Index".Loc(), ref adata.VenturePlanIndex);
                    }
                }
