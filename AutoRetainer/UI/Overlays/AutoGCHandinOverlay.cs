...
internal unsafe class AutoGCHandinOverlay : Window
{
    ...
    public AutoGCHandinOverlay() : base("AutoRetainer 筹备稀有品覆盖层", ImGuiWindowFlags.NoDecoration | ImGuiWindowFlags.AlwaysUseWindowPadding | ImGuiWindowFlags.AlwaysAutoResize | ImGuiWindowFlags.NoFocusOnAppearing | ImGuiWindowFlags.NoSavedSettings, true)
    {
    ...
    public override void Draw()
    {
        ...
        {
            ImGui.SameLine();
            ImGuiEx.Text(GradientColor.Get(ImGuiColors.DalamudRed, ImGuiColors.DalamudYellow), $"你可以使用“军票预支单”");
        }
        if(!Player.IsInHomeWorld)
        {
            ImGui.SameLine();
            ImGuiEx.Text(GradientColor.Get(ImGuiColors.DalamudRed, ImGuiColors.DalamudYellow), $"不在原始服务器时不会获得部队战绩。");
        }
        if(AutoGCHandin.Operation && Remaining != 0)
        {
            ImGui.SameLine();
            ImGuiEx.Text(ImGuiColors.DalamudViolet, $"剩余 {Remaining} 件");
        }
