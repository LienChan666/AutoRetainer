...
public sealed class VentureStatsManager
{
    ...
    internal void DrawVentures()
    {
        ...
        foreach(var cData in Data)
        {
            ...
            {
                if(ImGui.CollapsingHeader($"{Censor.Character(cData.Key)} | { "Total Ventures".Loc()}: {CharTotal.GetSafe(cData.Key)}###chara{cData.Key}"))
                {
                    display = true;
                }
            }
            ...
            foreach(var x in cData.Value)
            {
                ...
                if(display && num != 0)
                {
                    ...
                    ImGui.SameLine();
                    if(ImGui.CollapsingHeader($"{Censor.Retainer(x.Key)} | { "Ventures".Loc()}: {num}###{cData.Key}ret{x.Key}"))
                    {
