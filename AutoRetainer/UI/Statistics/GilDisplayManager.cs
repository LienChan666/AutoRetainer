...
public sealed class GilDisplayManager
{
    ...
    public void Draw()
    {
        ...
        foreach(var x in data)
        {
            ...
            foreach(var c in x.Value)
            {
                ...
                if(charTotal > C.MinGilDisplay)
                {
                    if(!C.GilOnlyChars)
                    {
                        ...
                        {
                            ImGuiEx.Text(ImGuiColors.DalamudYellow, $"        {"Free company".Loc()} {fcdata.Name}: {fcdata.Gil:N0}");
                        }
                    }
                    ImGuiEx.Text(ImGuiColors.DalamudViolet, $"    {Censor.Character(c.Name, c.World)}{(fcdata != null && fcdata.Gil > 0 ? "+FC" : "")} {"total".Loc()}: {charTotal:N0}");
            ...
            ImGuiEx.Text(ImGuiColors.DalamudOrange, $"{ "Data center total".Loc()} ({x.Key}): {dcTotal:N0}");
            globalTotal += dcTotal;
            ImGui.Separator();
            ImGui.Separator();
        }
        ImGuiEx.Text(ImGuiColors.DalamudOrange, $"{ "Overall total".Loc()}: {globalTotal:N0}");
    }
}
