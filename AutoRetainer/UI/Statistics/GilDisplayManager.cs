...
public sealed class GilDisplayManager
{
    ...
    public void Draw()
    {
        ...
        ImGui.InputInt("忽略金币低于该值的角色/雇员", ref C.MinGilDisplay.ValidateRange(0, int.MaxValue));
        ImGuiComponents.HelpMarker($"被忽略雇员的金币仍会计入角色/大区总计。仅当角色金币与其全部雇员金币都低于该值时，角色才会被忽略；被忽略角色不计入大区总计。");
        ...
        {
            ImGuiEx.Text($"{Lang.RegionName(x.Key)}：");
            var dcTotal = 0L;
            ...
            ImGuiEx.Text(ImGuiColors.DalamudOrange, $"大区总计（{Lang.RegionName(x.Key)}）：{dcTotal:N0}");
            globalTotal += dcTotal;
            ImGui.Separator();
            ImGui.Separator();
        }
