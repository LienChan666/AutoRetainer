...
public sealed class FcDataManager
{
    ...
    public void Draw()
    {
        ...
        if(ImGui.BeginTable("FCData", 5, ImGuiTableFlags.SizingFixedFit | ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg))
        {
            ...
            foreach(var x in C.FCData)
            {
                ...
                if(x.Value.LastGilUpdate != -1 && x.Value.LastGilUpdate != 0)
                {
                    ...
                    ImGuiEx.Tooltip($"{ "Last updated".Loc()} {UpdatedWhen(x.Value.LastGilUpdate)}. { "Ctrl + click to reset".Loc()}");
        ...
        string UpdatedWhen(long time)
        {
            ...
            if(diff < 1000L * 60 * 60) return $"{(int)(diff / 1000 / 60)} { "minute(s) ago".Loc()}";
            if(diff < 1000L * 60 * 60 * 60) return $"{(int)(diff / 1000 / 60 / 60)} { "hour(s) ago".Loc()}";
            return $"{(int)(diff / 1000 / 60 / 60 / 24)} { "day(s) ago".Loc()}";
        }
    }
