...
public sealed class FcDataManager
{
    ...
    public void Draw()
    {
        ImGui.Checkbox($"每 30 小时更新", ref C.UpdateStaleFCData);
        ImGui.SameLine();
        ...
        ImGui.SameLine();
        ImGui.Checkbox($"仅显示“钱包部队”数据", ref C.DisplayOnlyWalletFC);
        if(ImGui.BeginTable("FCData", 5, ImGuiTableFlags.SizingFixedFit | ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg))
        {
            ...
            foreach(var x in C.FCData)
            {
                ...
                foreach(var c in C.OfflineData.Where(z => z.FCID == x.Key))
                {
                    ...
                    if(ImGuiEx.HoveredAndClicked("左键 - 重登该角色"))
                    {
                ...
                if(x.Value.LastGilUpdate != -1 && x.Value.LastGilUpdate != 0)
                {
                    ...
                    ImGuiEx.Tooltip($"更新于{UpdatedWhen(x.Value.LastGilUpdate)}。按住 CTRL 并点击可重置");
                ...
                if(x.Value.FCPointsLastUpdate != 0)
                {
                    ...
                    ImGuiEx.Tooltip($"更新于{UpdatedWhen(x.Value.FCPointsLastUpdate)}");
                }
                else
                {
                    ImGuiEx.Text($"未知");
                ...
                ImGui.PopFont();
                ImGuiEx.Tooltip("将该部队标记为“钱包部队”。金币显示页会计入该部队的金币。");
                ImGui.SameLine();
                ...
                ImGuiEx.Tooltip($"按住 CTRL 并点击可删除该部队。若你再次登录该部队所属角色，它会重新出现。");
            }
